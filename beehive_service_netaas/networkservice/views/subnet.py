...
from beehive_service.plugins.computeservice.controller import ApiComputeVPC,\
    ApiComputeSubnet, ApiComputeService
...
class ipv6CidrBlockStateResponseSchema(Schema):
    ...
    statusMessage = fields.String(required=False, allow_none=True, example='', description='state of the CIDR block',
                                  validate=OneOf(['associating', 'associated', 'disassociating', 'disassociated',
                                                  'failing', 'failed']))
...
class SubnetItemParameterResponseSchema(Schema):
    assignIpv6AddressOnCreation = fields.Boolean(
        required=False,
        default=False)
    ...
    availableIpAddressCount = fields.Integer(
        required=False,
        allow_none=True)
    ...
    mapPublicIpOnLaunch = fields.Boolean(
    ...
    state = fields.String(
        ...
        validate=OneOf([getattr(ApiComputeSubnet.state_enum, x) for x in dir(ApiComputeSubnet.state_enum) if not x.startswith("__")]))
...
class DescribeSubnetsResponse1Schema(Schema):
    requestId = fields.String(
        ...
        example='',
        description='')
    subnetSet = fields.Nested(
        SubnetItemParameterResponseSchema,
        required=True,
        many=True,
        allow_none=False)
...
class DescribeSubnetsResponseSchema(Schema):
    DescribeSubnetsResponse = fields.Nested(
        DescribeSubnetsResponse1Schema,
        required=True,
        many=False,
        allow_none=False)
...
class DescribeSubnetsRequestSchema(Schema):
    ...
    subnet_id_N = fields.List(fields.String(example=''), required=False, allow_none=True, context='query',
                              collection_format='multi', data_key='subnet-id.N',
                              descriptiom=' ID of the subnet')
    SubnetId_N = fields.List(fields.String(example=''), required=False, allow_none=False,
                             context='query', collection_format='multi', data_key='SubnetId.N',
                             description=' ID of the subnet')
    vpc_id_N = fields.List(fields.String(example=''), required=False, allow_none=True,
    ...
    Nvl_MaxResults = fields.Integer(required=False, default=10, description='',
                                    data_key='Nvl-MaxResults', context='query')
    Nvl_NextToken = fields.String(required=False, default='0', description='',
                                  data_key='Nvl-NextToken', context='query')
...
class DescribeSubnets(ServiceApiView):
    ...
    tags = ['computeservice']
    definitions = {
        'DescribeSubnetsResponseSchema': DescribeSubnetsResponseSchema
    }
    ...
    responses = ServiceApiView.setResponses({
        200: {
            'description': 'success',
            'schema': DescribeSubnetsResponseSchema
        }
    })
...
class CreateSubnetApiParamRequestSchema(Schema):
    ...
    CidrBlock = fields.String(required=True, example='', description='subnet cidr like 10.102.78.0/24')
...
class CreateSubnet(ServiceApiView):
    ...
    responses = ServiceApiView.setResponses({
        202: {
            'description': 'success',
            'schema': CreateSubnetApiResponseSchema
        }
    })
    ...
    def post(self, controller, data, *args, **kwargs):
        ...
        inst = controller.add_service_type_plugin(service_definition.oid, account_id, name=name, desc=desc,
                                                  parent_plugin=inst_vpc, instance_config=data)
        inst.post_get()
